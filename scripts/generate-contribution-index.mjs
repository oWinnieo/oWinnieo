...
const messages = {
  zh: {
    heading: `#### Fork-aware 提交索引 · 最近 ${weeksToShow} 周`,
    note: "每个绿色方块代表该仓库当周由 @oWinnieo 创作、且可从默认分支追溯到的提交；颜色沿用 GitHub 官方贡献图的四级深浅。公开仓库方块可点击；私有仓库只显示 `private repo`，不暴露名称与链接。带 `fork` 标记的行会被完整计入。",
    repository: "仓库",
    fork: "fork",
  ...
  ja: {
    heading: `#### Fork対応コミット索引 · 直近${weeksToShow}週間`,
    note: "各緑色のマスは、その週に @oWinnieo が作成し、既定ブランチから追跡できるコミットを表す。濃淡にはGitHub公式の4段階コントリビューション配色を使う。公開リポジトリのマスはクリック可能。非公開の活動はリンクなしの `private repo` として匿名化する。`fork` 行もすべて集計対象。",
    repository: "リポジトリ",
    fork: "fork",
    private: "private repo",
